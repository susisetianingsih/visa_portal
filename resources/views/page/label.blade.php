    ...
    <main class="h-full overflow-y-auto">
          <div class="container px-6 mx-auto grid" x-data="liveSearch()">
            ...
            <div class="flex justify-between items-center my-6">
                <div
                  class="flex flex-col flex-wrap space-y-4 md:flex-row md:items-end md:space-x-4"
                >
                  <a href="{{ route('label_add') }}" class="block">
                    <button
            ...
            <div class="w-full mb-12 overflow-hidden rounded-lg shadow-xs" id="table-data">
              @include('page.partials.table_label', ['labels' => $labels])
            </div>
            ...
            <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150"
              ...
              <div class="mt-4 mb-6">
                ...
                <p class="text-sm text-gray-700 dark:text-gray-400">
                  Are you sure you want to delete label <span class="font-bold" x-text="dataName"></span>?
                </p>
              </div>
              <form x-bind:action="`{{ route('label_delete', '') }}/${dataId}`" method="POST">
                @method('delete')
                @csrf
                <footer
        ...
        <script>
            function liveSearch() {
                return {
                    search: '{{ $search }}',
                    fetchData() {
                        fetch(`{{ route('search_label') }}?search=${this.search}`, {
                            headers: {
