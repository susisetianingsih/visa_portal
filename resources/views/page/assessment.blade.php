    ...
    <main class="h-full overflow-y-auto">
          <div class="container px-6 mx-auto grid" x-data="liveSearch()">
            ...
            <div class="w-full mb-12 overflow-hidden rounded-lg shadow-xs" id="table-data">
              @include('page.partials.table_assessment', ['assessments' => $assessments])
            </div>
          ...
          <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0"
            ...
            <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150"
              ...
              <form x-bind:action="`{{ route('assessment_delete', '') }}/${dataId}`" method="POST">
                @method('delete')
                @csrf
                <footer
        ...
        <script>
            function liveSearch() {
                return {
                    search: '{{ $search }}',
                    fetchData() {
                        fetch(`{{ route('search_assessment') }}?search=${this.search}`, {
                            headers: {
