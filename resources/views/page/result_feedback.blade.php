        ...
        <div class="container px-6 mx-auto grid grid-cols-1 md:grid-cols-4 gap-4" x-data="{ currentQuestion: 0, totalQuestions: {{ $assessments->count() }}, currentLabelId: {{ $assessments->first()->label_id }},assessments: {{ $assessments->map(function ($assessment) { return ['label_id' => $assessment->label_id]; }) }}}" x-init="$watch('currentQuestion', value => currentLabelId = assessments[value].label_id)">
          <!-- Main Content -->
            <div class="col-span-1 md:col-span-3">
                ...
                @if ($result->visa == true)
                    <form x-ref="form" method="POST" action="{{ route('vendor_form_visa_feedback_post') }}">
                        @method('PUT')
                        @csrf
                                    ...
                                    <div class="flex flex-col">
                                        <div class="grid grid-cols-1 gap-x-6 gap-y-6 p-6 sm:p-12 sm:grid-cols-6">
                                            ...
                                            <div class="sm:col-span-6 min-w-0 p-4 bg-white rounded-lg shadow-xs border border-gray-200 bg-opacity-70 backdrop-blur-md">
                                                <label class="block text-sm font-medium text-gray-700" for="halodoc_comment">
                                                    <span class="text-lg font-semibold text-purple-700">Halodoc Comment</span>
                                                    <p class="mt-2 text-gray-700">
                                                        {{ $visa->halodoc_comment }}
                                                    </p>
                                                </label>
                                            </div>
                                            <div class="sm:col-span-6">
                                                <label class="block text-sm" for="vendor_feedback">
                                                <span class="text-gray-700 dark:text-gray-400">Vendor Feedback</span>
                                                <textarea name="vendor_feedback[]" id="vendor_feedback"
                                                    class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-textarea focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray"
                                                    rows="3" placeholder="Enter some long form content.">{{ old('vendor_feedback.' . $index) }}</textarea>
                                                <x-input-error :messages="$errors->get('vendor_feedback.' . $index)" class="mt-2" />
                                                </label>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="flex justify-between my-6">
                            ...
                            <button type="submit" x-show="currentQuestion === totalQuestions - 1" @click.prevent="openModal({{ Auth::user()->id }}, '{{ Auth::user()->username  }}')" class="px-4 py-2 text-sm font-medium text-white bg-purple-600 rounded-lg hover:bg-purple-700">
                                Submit
                            </button>                  
                        </div>
            ...
            <div class="md:block hidden col-span-1">
                ...
                <div x-show="currentQuestion === totalQuestions - 1" class="min-w-0 p-4 my-6 text-white bg-purple-600 rounded-lg shadow-xs">
                    ...
                    <p class="text-sm text-center mt-2">
                        Please, make sure your feedback is not empty! If any of them are still empty, the feedback will not be sent. 
                    </p>
                </div>
            </div>
        </div>
