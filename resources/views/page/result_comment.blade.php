@extends('layouts.app')
@section('title', 'Result Comment')
@section('content')
    <main class="h-full overflow-y-auto">
        <div class="container px-6 mx-auto grid grid-cols-1 md:grid-cols-4 gap-4" x-data="{ dataId: 0, currentQuestion: 0, totalQuestions: {{ $assessments->count() }}, currentLabelId: {{ $assessments->first()->label_id }},assessments: {{ $assessments->map(function ($assessment) { return ['label_id' => $assessment->label_id]; }) }}}" x-init="$watch('currentQuestion', value => currentLabelId = assessments[value].label_id)"
            <!-- Main Content -->
            <div class="col-span-1 md:col-span-3">
                <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
                    Comment for VISA Form of <span class="uppercase">{{ $result->user['username'] }}</span>
                </h2>

                @if ($result->visa == true)
                    <form x-ref="form" method="POST" x-bind:action="`{{ route('admin_result_comment_post', '') }}/${dataId}`">
                        @method('PUT')
                        @csrf
                        @foreach ($labels as $label)
                          ...
                          $num = 1;
                          @endphp
                          @foreach ($user_visa as $index => $visa)
                            @if ($visa->assessment->label_id == $label->id)
                              <div x-show="currentQuestion === {{ $index }}" id="question-{{ $visa->assessment_id }}">
                                  ...
                                  <div class="flex-1 w-full mx-auto bg-white rounded-lg shadow-xl dark:bg-gray-800">
                                      <div class="flex flex-col">
                                          <div class="grid grid-cols-1 gap-x-6 gap-y-6 p-6 sm:p-12 sm:grid-cols-6">
                                              <input type="hidden" name="assessment_id[]" value="{{ $visa->assessment_id }}">
                                              <div class="sm:col-span-6"> 
                                                  <label class="block text-sm" for="question">
                                                      ...
                                                      <p class="text-gray-700 mt-4 dark:text-gray-400">{{ $visa->assessment->question }}</p>
                                                  </label>
                                              </div>
                                              <div class="sm:col-span-3 border border-purple-500 p-3">
                                                  <label class="block text-sm" for="halodoc_expectation">
                                                      ...
                                                      <p class="text-gray-700 mt-2 dark:text-gray-400">{{ $visa->assessment->halodoc_expectation }}</p>
                                                  </label>
                                              </div>
                                              <div class="sm:col-span-3 border border-purple-500 p-3">
                                                  <label class="block text-sm" for="expected_evidence">
                                                      ...
                                                      <p class="text-gray-700 mt-2 dark:text-gray-400">{{ $visa->assessment->expected_evidence }}</p>
                                                  </label>
                                              </div>
                                              <div class="sm:col-span-6">
                                              ...
                                              <div class="sm:col-span-6">
                                                <label class="block text-sm" for="halodoc_comment">
                                                  <span class="text-gray-700 dark:text-gray-400">Halodoc Comment</span>
                                                  <textarea name="halodoc_comment[]" id="halodoc_comment"
                                                    class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-textarea focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray"
                                                    rows="3" placeholder="Enter some long form content.">{{ old('halodoc_comment.' . $index) }}</textarea>
                                                  <x-input-error :messages="$errors->get('halodoc_comment.' . $index)" class="mt-2" />
                                                </label>
                                              </div>
                                          </div>
                                      </div>
                        ...
                        <div class="flex justify-between my-6">
                            ...
                            <button type="submit" x-show="currentQuestion === totalQuestions - 1" @click.prevent="openModalComment({{ $result->user_id }})" class="px-4 py-2 text-sm font-medium text-white bg-purple-600 rounded-lg hover:bg-purple-700">
                              Submit
                            </button>                          
                        </div>
                        <!-- Modal backdrop. This what you want to place close to the closing body tag -->
                            <div x-show="isModalCommentOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0"
                                x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150"
                                x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                                class="fixed inset-0 z-30 flex items-end bg-black bg-opacity-50 sm:items-center sm:justify-center">
                                <!-- Modal -->
                                <div x-show="isModalCommentOpen" x-transition:enter="transition ease-out duration-150"
                                x-transition:enter-start="opacity-0 transform translate-y-1/2" x-transition:enter-end="opacity-100"
                                x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
                                x-transition:leave-end="opacity-0  transform translate-y-1/2" @click.away="closeModal"
                                @keydown.escape="closeModal"
                                class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl"
                                role="dialog" id="commentmodal">
                                ...
                                <div class="mt-4 mb-6">
                                    ...
                                    Are you sure you want to submit the comments?
                                    </p>
                                </div>
                                <footer
            ...
            <div class="md:block hidden col-span-1">
                ...
                <form x-ref="form" x-bind:action="`{{ route('admin_result_comment_enough', '') }}/${dataId}`" method="POST">
                    ...
                    <div class="my-6">
                        <button
                            @click.prevent="openModal({{ $result->user_id }}, '{{ $result->user['username']  }}')"
                            type="submit"
                            class="flex items-center justify-center w-full px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 rounded-lg hover:bg-purple-700 focus:shadow-outline-purple focus:outline-none active:bg-purple-600 {{ request()->is('*/register-halodoc') ? 'border' : 'border-transparent' }} text-center"
                        >
                            End Session
                        </button>
                    </div>
                ...
                <div x-show="currentQuestion === totalQuestions - 1" class="min-w-0 p-4 my-6 text-white bg-purple-600 rounded-lg shadow-xs">
                    <h2 class="text-xl text-center font-semibold uppercase">Attention!</h2>
                    <p class="text-sm text-center mt-2">
                      Please, make sure your comment is not empty! If any of them are still empty, the comment will not be sent. 
                    </p>
                </div>
            </div>
        </div>
